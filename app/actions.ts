...
export async function getFlights(
  ...
  const mappedFlights: FlightViewModel[] = rawFlights.flatMap((f) => {
    ...
    const from_airport =
      direction === "D" ? f.scanned_airport : f.related_airport;
    const to_airport =
      direction === "D" ? f.related_airport : f.scanned_airport;
    ...
    const baseFlight: FlightViewModel = {
      ...
      gate: f.gate,
      gate_status_code: f.gate_status_code,
      gate_status_desc: f.gate_status_desc,
